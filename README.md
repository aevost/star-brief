...
这个 skill 会引导 Codex 读取项目规格文档与近期代码改动，并按 STAR 结构生成一份中文项目说明文档，适合非技术背景的管理层快速理解项目背景、目标、关键动作与结果。
...
2. Extract it.
3. Rename or place the folder as `star-exec-brief`.
...
- The skill logic lives in `SKILL.md`.
- `agents/openai.yaml` provides UI metadata for Codex.
- `README.md` is only for human readers and GitHub presentation; it does not change the skill behavior.
